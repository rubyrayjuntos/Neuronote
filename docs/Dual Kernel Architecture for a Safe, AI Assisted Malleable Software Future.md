...
To execute dynamic dataflow logic, the system employs a **WebAssembly (WASM) Sandbox** running QuickJS.  
...
• **Tier 1 Execution:** Pure, synchronous operators (e.g., `Math.*`, `Text.*`) execute within the QuickJS sandbox. They are mathematically guaranteed to be side-effect free and deterministic.  
• **Tier 2 Execution:** Heavy, asynchronous operators (e.g., `Image.Vectorize`, `Audio.FFT`) run in the native Worker scope but are wrapped in a governance shell that enforces strict Time-to-Live (TTL) and memory quotas.  
...
• **Tier 1 (Pure/Sync):** 33 operators (e.g., `Text.RegexMatch`, `Math.Clamp`, `List.Map`). These run inside the QuickJS sandbox and are mathematically guaranteed to be side-effect free and deterministic \[Source 413\].  
• **Tier 2 (Metered/Async):** 13 heavy operators (e.g., `Image.Vectorize`, `Audio.FFT`). These run in a dedicated Web Worker. To prevent resource exhaustion, these operators are wrapped in a governance shell that enforces strict time-to-live (TTL) and memory quotas \[Source 413, 1362\].  
